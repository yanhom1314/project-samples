...
trait UserRepository extends BaseRepository[User] {
  @SqlUpdate("create table if not exists j_user (id bigint primary key auto_increment, username varchar(100),password varchar(100),age int,address varchar(100))")
  override def createTable(): Unit

  @SqlUpdate("insert into j_user(username, password, age, address) values(:username, :password, :age, :address)")
